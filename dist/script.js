...
document.addEventListener("DOMContentLoaded", () => {
        ...
        if (totalBalanceElement) {
            ...
            if (dynamicTextElement) {
                if (totalBalance == 0) {
                    ...
                    dynamicTextElement.innerHTML = "<b>Status Keuanganmu:</b> <b class = 'positiveValue';>Tjakep</b>";
                }
                else if (totalBalance > 1000000) {
                    dynamicTextElement.innerHTML = "<b>Status Keuanganmu:</b> <b class = 'positiveValue';>Gud</b>"; // Display if total balance is greater than -10000
                }
                else if (totalBalance > 0.000001) {
                    dynamicTextElement.innerHTML = "<b>Status Keuanganmu:</b> <b class = 'positiveValue';>Oke kok</b>"; // Display if total balance is greater than -10000
                }
                else if (totalBalance < -100000000) {
                    dynamicTextElement.innerHTML = "<b>Status Keuanganmu:</b> <b class = 'negativeValue';>KACAU BANGET BOSS</b>"; // Display if total balance is greater than -10000
                }
                else if (totalBalance < -10000000) {
                    dynamicTextElement.innerHTML = "<b class = 'negativeValue';>Status Keuanganmu:</b> <b class = 'negativeValue';>GWS DAH</b>"; // Display if total balance is greater than -1000
                }
                else if (totalBalance < -1000000) {
                    dynamicTextElement.innerHTML = "<b class = 'negativeValue';>Status Keuanganmu:</b> <b class = 'negativeValue';>WADUCH </b>"; // Display if total balance is greater than -500
                }
                else if (totalBalance < -100000) {
                    dynamicTextElement.innerHTML = "<b class = 'negativeValue';>Status Keuanganmu:</b> <b class = 'negativeValue';>Bahaya bro</b>";
                }
                else if (totalBalance < -0.000000001) {
                    dynamicTextElement.innerHTML = "<b class = 'negativeValue';>Status Keuanganmu:</b> <b class = 'negativeValue';>Coba deh Berhemat</b>"; // Display if total balance is greater than -100
                }
                else {
                    dynamicTextElement.innerHTML = "<b>Status Keuanganmu:</b>"; // Clear the dynamic text if total balance is positive or above -100
                }
            }
        }
    }
    ...
    transactionForm === null || transactionForm === void 0 ? void 0 : transactionForm.addEventListener("submit", (event) => {
        event.preventDefault();
        const dateInput = document.getElementById("date");
        const itemInput = document.getElementById("item");
        const amountInput = document.getElementById("amount");
        const typeInput = document.getElementById("type"); // Get the type input
        const date = dateInput.value;
        const item = itemInput.value;
        const amount = parseFloat(amountInput.value);
        const type = typeInput.value; // Ensure the type value is one of the specified options
        if (!date || !item || isNaN(amount)) {
            alert("Please fill in all fields correctly.");
            return;
        }
        // Create a new transaction
        const transaction = { date, item, amount, type };
    ...
    function deleteTransaction(transaction) {
        const existingTransactions = getTransactionsFromStorage();
        const updatedTransactions = existingTransactions.filter((t) => !(t.date === transaction.date && t.item === transaction.item && t.amount === transaction.amount));
